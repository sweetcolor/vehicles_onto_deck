...
RSpec.describe MainPageController, :type => :controller do
  describe 'GET #query' do
    it 'responds successfully with an HTTP 200 status code' do
      ...
      assigns(:parsed_params).should( [[:deck_width, 5], [:deck_length, 8], [:stdmax, 500], [:EX, {{:width=>0..6, :length=>0..1}=>300, {:width=>0..0, :length=>4..4}=>200}], [:sv, [2, 3, 100]], [:rv, [{:name=>"B", :width=>3, :length=>5, :height=>170}, {:name=>"C", :width=>2, :length=>3, :height=>150}, {:name=>"A", :width=>2, :length=>3, :height=>150}, {:name=>"D", :width=>2, :length=>3, :height=>100}]], [:sort_order, ["L", "W", 1]], [:placement, "UL"], [:a, ["vis", "shc0,0,0,0"]], [:LL, [3, 0, 162, 232]], [:c, "y"], [:SV, [{:name=>:sv, :width=>2, :length=>3, :height=>100}]]])
      # expect(controller.parsed_params).to eq({:deck_width=>5, :deck_length=>8, :stdmax=>500, :EX=>{{:width=>0..6, :length=>0..1}=>300, {:width=>0..0, :length=>4..4}=>200}, :sv=>[2, 3, 100], :rv=>[{:name=>"B", :width=>3, :length=>5, :height=>170}, {:name=>"C", :width=>2, :length=>3, :height=>150}, {:name=>"A", :width=>2, :length=>3, :height=>150}, {:name=>"D", :width=>2, :length=>3, :height=>100}], :sort_order=>["L", "W", 1], :placement=>"UL", :a=>["vis", "shc0,0,0,0"], :LL=>[3, 0, 162, 232], :c=>"y", :SV=>[{:name=>:sv, :width=>2, :length=>3, :height=>100}]})
    end
  end
end

# http://0.0.0.0:3000/deck_width=5~deck_length=8~stdmax=500~EX_A1:B7=300_E1:E1=200~sv=2,3,100~rv_A,2,3,150_B,3,5,170_C,2,3,150_D,2,3,100~sort_order=L,W,1~placement=UL~a=vis=shc0,0,0,0~LL=3,0,162,232~c=y
# http://0.0.0.0:3000/deck_width=20~deck_length=300~stdmax=500~EX_A1:B13=300_J1:J1=200~sv=2,3,100~rv_V%251,5,89,5_V%252,5,50,5_V%253,5,55,5_V%254,5,135,5_V%255,5,175,5_V%256,5,50,5_V%257,5,50,5_V%258,5,50,5_V%259,5,70,5~sort_order=L,W,1~placement=UL~a=vis=shc0,0,0,0~LL=3,0,162,232~c=n
# http://0.0.0.0:3000/deck_width=20~deck_length=300~stdmax=500~EX_A1:B13=300_J1:J1=200~sv=2,3,100~rv_V%251,5,89,5_V%252,5,50,5_V%253,5,55,5_V%254,5,135,5_V%255,5,175,5_V%256,5,50,5_V%257,5,50,5_V%258,5,50,5_V%259,5,70,5~sort_order=L,W,1~placement=UL~a=vis=shc0,0,0,0~LL=3,0,162,232~c=n
...
# http://0.0.0.0:3000/deck_width=20~deck_length=200~stdmax=500~EX_A1:B13=300_J1:J1=200~sv=2,3,100~rv_V%251,5,89,5_V%252,5,50,5_V%253,5,55,5_V%254,5,135,5_V%255,5,175,5_V%256,5,50,5_V%257,5,50,5_V%258,5,50,5_V%259,5,70,5~sort_order=L,W,1~placement=UL~a=vis=shc0,0,0,0~LL=3,0,162,232~c=n
